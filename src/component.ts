...
export class Props {
    playerWidth = prop<string | number>({ default: 1280 });
    playerHeight = prop<string | number>({ default: 720 });
    playerParameters = prop<YT.PlayerVars>({ default: { autoplay: 0, time: 0, mute: 0 } });
    videoId = prop<string>({ default: '' });
    noCookie = prop<boolean>({ default: true });
}
...
export default class Player extends Vue.with(Props) {
     ...
     * @param suggestedQuality Suggested video player quality
     */
    cueVideoByUrl(mediaContentUrl: string, startSeconds?: number, suggestedQuality?: YT.SuggestedVideoQuality): void {
